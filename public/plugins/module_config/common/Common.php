...
class Common
{
    ...
    {
        if( !is_array($param) ){
            return  $this->debugEcho("模块调用($symbol):".'参数应为索引数组形式' );
        }
        //读取本模块的接口配置数据
        ...
        if( !is_array( $demandConfig ) || !isset( $demandConfig [$id] ) ){
            return $this->debugEcho( "模块调用($symbol):".'未获取到数据，检查标识、id是否传递正确' );
        }
        $config = $demandConfig [$id];
        if( !isset( $config['api'] ) || empty( $config['api'] ) ){
            return $this->debugEcho( "模块调用($symbol):".'未获取到api地址，去配置模块检查是否已配置了接口地址' );
        }
        $api = $config['api'];
        ...
        if( $res == -1 ){
            return $this->debugEcho( "模块调用($symbol):".'接口地址不合法,错误地址:'.$api );
        }else if( $res == 0 ){
			return $this->debugEcho( "模块调用($symbol):".'类不存在:'.$this->classExists($api,true) );
        }else{
        	$resData = $this->methodExistsCheck( $res['class'] ,$res['method'] );
        	if( $resData == 0 ){
        		return $this->debugEcho("模块调用($symbol):".'方法不存在：'.$resData['method'] );
        	}
        }
        $class = $res['class'];
        $method = $res['method'];
    ...
    {
        return  zy_array( false , $str , null , 104 , true);
    }
