...
class CheckFileModel 
{
	...
	public static function checkDemandModuleFileExist( $symbol = '' )
	{
		...
		foreach ( $content as $key => $value ) {
			$path = $path.DS.cmf_parse_name( $value [ 'demandSymbol' ] );
			if( !file_exists( $path ) ){
				$isNoExist .= '<div>所需模块：'.$value['demandName'].'('.$value['demandSymbol'].')'.'不存在</div>' ;
			}else{
				$isExist .= '<div>所需模块：'.$value['demandName'].'('.$value['demandSymbol'].')'.'存在</div>'  ;
			}
		}
	...
	public static function checkDemandDbTableExist( $symbol = '' )
	{
		...
		foreach ($config as $key => $value) {
			...
			foreach ($tables as $tableName ) {
				$path = $path.DS.cmf_parse_name( $key ).DS.'data'.DS.$tableName.'.sql';
				if( !file_exists( $path ) ){
