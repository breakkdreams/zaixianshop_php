...
class AdminIndexController extends PluginAdminBaseController
{
    ...
    {
        $data = ['remark'=>'add','table_data'=>['test_field'=>'这是测试数据!']];
        hook( 'add_remind' ,  $data );
        ...
        if($appListData['status']=='error'){
            throw new \Exception('ERROR:'.$appListData['message']." ErrorCode:".$appListData['code'], 1);
        }                
        ...
        $moduleList = $appListData['data']['moduleList'];

        $totalPgN = ($pages[ 'current_page' ]-1) * 20 + 1;
        $this->assign( 'totalPgN' , $totalPgN );
        //分页
        $ul = $this->pages( $pages );
    ...
    {   
        ...
        $pluginInstallObj = new \app\admin\controller\PluginController();
        $pluginInstallObj->pluginModuleId = $moduleId;
        $res = $pluginInstallObj -> updatePlugin( $module['name'] );

        if( isset( $res['status'] ) && $res['status'] == 'error'  ){
            return zy_json_echo( false ,  $res['message'] , null , 102 );
        }
        if( empty( $res ) || -1 == $res ){
            return zy_json_echo( false , '未知错误,更新失败,请稍后再试!' , null , 103 );
        }
        return zy_json_echo( true , '更新成功!' );
    }
