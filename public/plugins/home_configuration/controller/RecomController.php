...
class RecomController extends PluginAdminBaseController
{
    ...
    {   
        ...
        $data = Db::name('home_configuration_recom')->where('type',1)->where('recom_name','like','%'.$keyword.'%')->order('sort','asc')->paginate(10)->each(function($item,$key){
    ...
    {   
        ...
        $data = Db::name('home_configuration_recom')->where('type',2)->where('recom_name','like','%'.$keyword.'%')->order('sort','asc')->paginate(10)->each(function($item,$key){
                $return = $this->getDomain();
                if(empty($item['img_url'])){
                    $item['img_url'] = $return."/plugins/home_configuration/view/public/default/hhh.png";
                }else{
    ...
    {   
        $param = $this->request->param();
        $add['recom_name'] = $param['name'];
        $add['type'] = $param['type'];
        $add['img_url'] = $param['picture'];
        $add['jump_url'] = $param['jump'];
        $add['status'] = $param['status'];
        $best_sort = Db::name('home_configuration_recom')->where('type',$param['type'])->order('sort','desc')->find();
        if(!empty($best_sort['sort'])){
            $add['sort'] = $best_sort['sort']+1;
        }else{
            $add['sort'] = 1;
        }
        $re_data = Db::name('home_configuration_recom')->where('recom_name',$add['recom_name'])->find();
        if(!empty($re_data)){
            return zy_json_echo(false,'该名称已被使用，不可重复添加',null,100);
        }
        $re = Db::name('home_configuration_recom')->insert($add);
        if(empty($re)){
     ...
     public function editRecomPage()
    {   
        $param = $this->request->param();
        $data = Db::name('home_configuration_recom')->where('id',$param['id'])->find();
        if(!empty($data)){
            $return = $this->getDomain();
    ...
    {   
        ...
        $re_data = Db::name('home_configuration_recom')->where('recom_name',$update['recom_name'])->find();
        if(!empty($re_data)){
            if($re_data['id']!=$update['id']){
                return zy_json_echo(false,'该名称已被使用，不可重复添加',null,100);
            }
        }
        $re = Db::name('home_configuration_recom')->update($update);
        if(empty($re)){
    ...
    {   
        ...
        $re = Db::name('home_configuration_recom')->where('id',$param['id'])->delete();
        if(empty($re)){
    ...
    public function allDelete(){
        ...
        foreach($ids as $key=>$value){
            $re ='';
            $re = Db::name('home_configuration_recom')->where('id',$value)->delete();
            if(empty($re)){
...
//----------------------------------------------------------------------------------------排序操作
    ...
    */
    public function uprwImage(){
        // $data=parent::getRequestParam();
        $data = $this->request->param();
        // return $data;
        // return 'A';
            // return zy_json_echo(false,'A',null,110);
