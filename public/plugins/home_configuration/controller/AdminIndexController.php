...
class AdminIndexController extends PluginAdminBaseController
{
    ...
    {   
        ...
        $data = Db::name('home_configuration_skin')->where('type',1)->where('skin_name','like','%'.$keyword.'%')->order('sort','asc')->paginate(10)->each(function($item,$key){
    ...
    {   
        ...
        $data = Db::name('home_configuration_skin')->where('type',2)->where('skin_name','like','%'.$keyword.'%')->order('sort','asc')->paginate(10)->each(function($item,$key){
           		$return = $this->getDomain();
                if(empty($item['img_url'])){
                    $item['img_url'] = $return."plugins/home_configuration/view/public/default/hhh.png";
                }else{
        ...
        $this->assign('data',$data);
        $this->assign('keyword',$keyword);
        return $this->fetch('webSkin');
    }


    //添加皮肤页面
    public function addSkinPage()
    {   
    	$param = $this->request->param();
    	$type=$param['type'];
    	$this->assign('type',$type);
        return $this->fetch('addSkinPage');
    }
    ...
    {   
        $param = $this->request->param();
        $add['skin_name'] = $param['name'];
        $add['type'] = $param['type'];
        $return = $this->getDomain();
        $add['img_url'] = $param['picture'];
        $add['status'] = $param['status'];
        $best_sort = Db::name('home_configuration_skin')->where('type',$param['type'])->order('sort','desc')->find();
        if(!empty($best_sort['sort'])){
            $add['sort'] = $best_sort['sort']+1;
        }else{
            $add['sort'] = 1;
        }
        $re_data = Db::name('home_configuration_skin')->where('skin_name',$add['skin_name'])->find();
        if(!empty($re_data)){
            return zy_json_echo(false,'该名称已被使用，不可重复添加',null,100);
        }
        $re = Db::name('home_configuration_skin')->insert($add);
        if(empty($re)){
     ...
     public function editSkinPage()
    {   
        $param = $this->request->param();
        $data = Db::name('home_configuration_skin')->where('id',$param['id'])->find();
        $return = $this->getDomain();
    ...
    {   
        $param = $this->request->param();
        $update['id'] = $param['id'];
        $update['skin_name'] = $param['name'];
        $update['type'] = $param['type'];
        $update['status'] = $param['status'];
        if(!empty($param['picture'])){
            $update['img_url'] =$param['picture'];
        }
        $re_data = Db::name('home_configuration_skin')->where('skin_name',$update['skin_name'])->find();
        if(!empty($re_data)){
            if($re_data['id']!=$update['id']){
                return zy_json_echo(false,'该名称已被使用，不可重复添加',null,100);
            }
        }
        $re = Db::name('home_configuration_skin')->update($update);
        if(empty($re)){
    ...
    {   
        ...
        $re = Db::name('home_configuration_skin')->where('id',$param['id'])->delete();
        if(empty($re)){
    ...
    public function allDelete(){
        ...
        foreach($ids as $key=>$value){
            $re ='';
            $re = Db::name('home_configuration_skin')->where('id',$value)->delete();
            if(empty($re)){
