...
class Upload
{
	...
	{
		if( empty( $this->rootPath ) ) return $this->result( false , '未定义文件存放路径！');
        ...
        try {
        	...
        	return  $this->result(false,'创建文件异常：'.$e->getMessage() );
        }
        ...
        if(!isset($exts['extension'])){
            ...
            return  $this->result(false,'文件格式不合法。');
        ...
        if( !in_array( $exts['extension'] , $this->exts ) ){
            ...
            return   $this->result( false , '文件允许格式：'.implode( '|', $this->exts ) );
        ...
        if( $this->size <= $this->fileSize ){
            ...
            return  $this->result(false,'文件大小不超过：'.$this->size.'MB,请检查!');
	 ...
	 */
	private function result( $status = false , $message = '' , $data = '' )
	{
