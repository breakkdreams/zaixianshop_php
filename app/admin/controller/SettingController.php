...
class SettingController extends AdminBaseController
{
    ...
    public function receive(){
        if($this->request->isPost()){
            ...
            if(Db::name('receive')->find()){
                Db::name('receive')->where('id',1)->update($post);
            }else{
                Db::name('receive')->insert($post);
            }
        }
        $info=Db::name('receive')->find();
