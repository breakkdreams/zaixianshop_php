...
class Baksql {
     ...
     * @param array $data
     */
    private function writeToFile($tables = array(), $ddl = array(), $data = array(),$path,$fileName)
    {
        $this->config['path']=$path;
        $this->config['sqlbakname']=$fileName;
     ...
     * @return bool
     */
    public function backup($tables = array(),$path,$fileName)
    {
        //存储表定义语句的数组
        $ddl = array();
        //存储数据的数组
        ...
        if (!empty($this->tables))
        {
            ...
            //开始写入
            return $this->writeToFile($this->tables, $ddl, $data,$path,$fileName);
        }
        else
        {
            $this->error = '数据库中没有表!';
